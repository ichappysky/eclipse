...
public class HelpCountExpressions {
	public static int getWays(String s, boolean target, int start, int end,
		...
		if (map.containsKey(key)) {
			...
			for (int i = start + 1; i < end; i += 2) {
				...
				if (curOp == '&') {
					count += getWays(s, true, start, i - 1, map)
							* getWays(s, true, i + 1, end, map);
				} else if (curOp == '|') {
					int left = total((i - start) / 2);
					int right = total((end - i) / 2);
					count += left * right - getWays(s, false, start, i - 1, map)
							* getWays(s, false, i + 1, end, map);
				} else if (curOp == '^') {
					count += getWays(s, true, start, i - 1, map)
							* getWays(s, false, i + 1, end, map)
							+ getWays(s, false, start, i - 1, map)
							* getWays(s, true, i + 1, end, map);
				}
			}
		}
		...
		return total((end - start) / 2) - count;
	}
	...
	public static void main(String[] args) {
		...
		System.out.println(getWays(terms, result, 0, terms.length() - 1,
